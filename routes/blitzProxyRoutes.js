...
router.post('/search-orders', async (req, res) => {
  try {
      ...
      const promises = batch.map(async (merchantOrderId) => {
        try {
          ...
          if (response.data.results && response.data.results.length > 0) {
            const orderData = response.data.results[0];
            ...
            console.log(`   ✅ Found: ${merchantOrderId}`);
          } else {
    ...
    console.log(`✅ Found ${foundCount} orders in Blitz`);
