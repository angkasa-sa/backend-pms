...
const existingRecord = await ExcelData.findOne(filter).session(session);
if (existingRecord && (existingRecord["Receiving Time"] || existingRecord["ETA"])) {
const deliveryStatus = existingRecord.calculateDeliveryStatus();
if (deliveryStatus && deliveryStatus !== existingRecord["Delivery Status"]) {
await ExcelData.updateOne(
filter,
{ $set: { "Delivery Status": deliveryStatus } },
{ session }
);
}
}
