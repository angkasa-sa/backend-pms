...
submittedOn: String(item.submittedOn || '').trim(),
respondents: String(item.respondents || '').trim(),
tanggalKeluarUnit: String(item.tanggalKeluarUnit || '').trim(),
merkUnit: String(item.merkUnit || '').trim(),
merkUnitLainnyaText: String(item.merkUnitLainnyaText || '').trim(),
nomorPlat: String(item.nomorPlat).trim().toUpperCase(),
picPenanggungjawab: String(item.picPenanggungjawab || '').trim(),
picPenanggungjawabLainnyaTeks: String(item.picPenanggungjawabLainnyaTeks || '').trim(),
namaLengkapUserSesuaiKtp: String(item.namaLengkapUserSesuaiKtp).trim(),
nikUser: String(item.nikUser || '').trim(),
nomorSimCUser: String(item.nomorSimCUser || '').trim(),
alamatLengkapUser: String(item.alamatLengkapUser || '').trim(),
kotaTempatTinggalUser: String(item.kotaTempatTinggalUser || '').trim(),
kotaTempatTinggalUserLainnyaText: String(item.kotaTempatTinggalUserLainnyaText || '').trim(),
kelengkapanUnit: String(item.kelengkapanUnit || '').trim(),
kelengkapanUnitAksesorisText: String(item.kelengkapanUnitAksesorisText || '').trim(),
catatanTambahan: String(item.catatanTambahan || '').trim(),
ktpAsli: String(item.ktpAsli || '').trim(),
simCAsli: String(item.simCAsli || '').trim(),
kk: String(item.kk || '').trim(),
skck: String(item.skck || '').trim(),
suratKeteranganDomisili: String(item.suratKeteranganDomisili || '').trim(),
suratPernyataanPeminjaman: String(item.suratPernyataanPeminjaman || '').trim(),
bast: String(item.bast || '').trim(),
fotoUnitBersamaUser: String(item.fotoUnitBersamaUser || '').trim(),
noTelpUser: String(item.noTelpUser || '').trim(),
namaProject: String(item.namaProject || '').trim(),
stnkUnit: String(item.stnkUnit || '').trim()
});
...
return {
$or: [
{ nomorPlat: searchRegex },
{ merkUnit: searchRegex },
{ namaLengkapUserSesuaiKtp: searchRegex },
{ nikUser: searchRegex },
{ noTelpUser: searchRegex },
{ kotaTempatTinggalUser: searchRegex },
{ namaProject: searchRegex }
]
};
};
...
if (filters.merkUnit) {
query.merkUnit = new RegExp(filters.merkUnit.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
}

if (filters.kotaTempatTinggalUser) {
query.kotaTempatTinggalUser = new RegExp(filters.kotaTempatTinggalUser.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
}

if (filters.namaProject) {
query.namaProject = new RegExp(filters.namaProject.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
}
...
query.$or = [
{ nomorSimCUser: { $exists: false } },
{ nomorSimCUser: '' },
{ nomorSimCUser: '-' },
{ nomorSimCUser: { $regex: /^.{0,6}$/ } }
];
}
}
...
if (sortKey === 'createdAt' || sortKey === 'tanggalKeluarUnit') {
...
try {
const [merkUnits, kotas, projects] = await Promise.all([
FleetData.distinct('merkUnit', { merkUnit: { $ne: '', $exists: true } }),
FleetData.distinct('kotaTempatTinggalUser', { kotaTempatTinggalUser: { $ne: '', $exists: true } }),
FleetData.distinct('namaProject', { namaProject: { $ne: '', $exists: true } })
]);

const activeCount = await FleetData.countDocuments({
nomorSimCUser: { 
$exists: true, 
$ne: '', 
$ne: '-', 
...
const totalCount = await FleetData.countDocuments();

res.status(200).json({
merkUnits: merkUnits.sort(),
kotas: kotas.sort(),
...
try {
const nomorPlat = req.params.plat;
console.log(`Mencari data fleet untuk nomor plat: ${nomorPlat}`);

const data = await FleetData.find({ 
nomorPlat: { $regex: new RegExp(nomorPlat, "i") } 
}).lean();

if (!data || data.length === 0) {
return res.status(404).json({
message: `Data fleet untuk nomor plat ${nomorPlat} tidak ditemukan`,
count: 0,
data: []
});
}

console.log(`Data fleet ditemukan untuk nomor plat ${nomorPlat}: ${data.length} records`);

res.status(200).json({
message: `Data fleet untuk nomor plat ${nomorPlat} berhasil diambil`,
count: data.length,
data: data
});
} catch (error) {
console.error(`Get fleet by plat error:`, error.message);
res.status(500).json({ 
message: `Gagal mengambil data fleet berdasarkan nomor plat`, 
error: error.message 
});
}
};
