...
const uploadSayurboxData = async (req, res) => {
    try {
        ...
        console.log("Uploading sayurbox data...");

        await SayurboxData.deleteMany({});
        console.log("Data sayurbox lama dihapus");
        ...
        const inserted = await SayurboxData.insertMany(transformedData);
...
const appendSayurboxData = async (req, res) => {
    try {
        ...
        console.log("Appending sayurbox data...");
        ...
        const bulkOps = transformedData.map(item => ({
            updateOne: {
                filter: { orderNo: item.orderNo, hubName: item.hubName, driverName: item.driverName },
                update: { $set: item },
                upsert: true
            }
        }));

        const result = await SayurboxData.bulkWrite(bulkOps);
...
const getAllSayurboxData = async (req, res) => {
    try {
        const data = await SayurboxData.find().sort({ hubName: 1, driverName: 1, deliveryDate: -1 });
...
const getSayurboxDataByHub = async (req, res) => {
    try {
        ...
        console.log("Jumlah data sayurbox ditemukan:", data.length);
...
const compareDataSayurbox = async (req, res) => {
    try {
        ...
        try {
            const sayurboxData = await SayurboxData.find({}, { orderNo: 1, totalWeightPerorder: 1, distanceInKm: 1 }).lean();
