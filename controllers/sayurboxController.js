...
const DISPLAY_CONFIG = {
    UNMATCHED_DISPLAY_LIMIT: 200,
    MAX_TOTAL_DISPLAY: 500
};
...
const compareDataSayurbox = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const displayLimit = DISPLAY_CONFIG.UNMATCHED_DISPLAY_LIMIT;
            const maxTotalDisplay = DISPLAY_CONFIG.MAX_TOTAL_DISPLAY;

            const exceedsDisplayLimit = (unmatchedExcelCodes.length + unmatchedSayurboxCodes.length) > maxTotalDisplay;
            const actualExcelDisplayLimit = exceedsDisplayLimit ? 
                Math.min(displayLimit, Math.floor(maxTotalDisplay * 0.6)) : 
                Math.min(displayLimit, unmatchedExcelCodes.length);
            const actualSayurboxDisplayLimit = exceedsDisplayLimit ? 
                Math.min(displayLimit, maxTotalDisplay - actualExcelDisplayLimit) : 
                Math.min(displayLimit, unmatchedSayurboxCodes.length);
            ...
            res.status(200).json({
                ...
                summary: {
                    ...
                    success: true,
                    displayInfo: {
                        excelDisplayed: Math.min(actualExcelDisplayLimit, unmatchedExcelCodes.length),
                        excelTotal: unmatchedExcelCodes.length,
                        sayurboxDisplayed: Math.min(actualSayurboxDisplayLimit, unmatchedSayurboxCodes.length),
                        sayurboxTotal: unmatchedSayurboxCodes.length,
                        displayLimit: displayLimit,
                        maxTotalDisplay: maxTotalDisplay
                    }
                },
                unmatchedExcelCodes: unmatchedExcelCodes.slice(0, actualExcelDisplayLimit),
                unmatchedSayurboxCodes: unmatchedSayurboxCodes.slice(0, actualSayurboxDisplayLimit),
                hasMoreUnmatchedExcel: unmatchedExcelCodes.length > actualExcelDisplayLimit,
                hasMoreUnmatchedSayurbox: unmatchedSayurboxCodes.length > actualSayurboxDisplayLimit
            });
